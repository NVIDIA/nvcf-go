...
func AddRequestInfoToContext(req *http.Request) *http.Request {
	ctx := req.Context()
	ctx = context.WithValue(ctx, HandlerTypeCustom, HandlerTypeCustom)
	ctx = context.WithValue(ctx, HeaderKeyRequestID, req.Header.Get(HeaderKeyRequestID))
	ctx = context.WithValue(ctx, HeaderKeyAuditID, req.Header.Get(HeaderKeyAuditID))
	ctx = context.WithValue(ctx, HeaderETag, req.Header.Get(HeaderETag))
	ctx = context.WithValue(ctx, b3.TraceID, req.Header.Get(b3.TraceID))
	ctx = context.WithValue(ctx, b3.SpanID, req.Header.Get(b3.SpanID))
	ctx = context.WithValue(ctx, b3.ParentSpanID, req.Header.Get(b3.ParentSpanID))
	ctx = context.WithValue(ctx, b3.Flags, req.Header.Get(b3.Flags))
	ctx = context.WithValue(ctx, b3.Sampled, req.Header.Get(b3.Sampled))
