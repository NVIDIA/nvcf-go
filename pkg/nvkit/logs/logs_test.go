...
func TestGetZapFieldsFromContext_CustomHandler(t *testing.T) {
	ctx := context.Background()
	ctx = context.WithValue(ctx, serverutils.HandlerTypeCustom, serverutils.HandlerTypeCustom)
	ctx = context.WithValue(ctx, serverutils.HeaderKeyRequestID, "req-abc-123")
	fields := GetZapFieldsFromContext(ctx)
