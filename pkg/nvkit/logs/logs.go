...
func GetZapFieldsFromContext(ctx context.Context) []zap.Field {
	...
	if val := ctx.Value(serverutils.HandlerTypeCustom); val != nil {
		if val := ctx.Value(serverutils.HeaderKeyRequestID); val != nil {
			fields = append(fields, zap.String(KeyRequestID, val.(string)))
		}
		return fields
	}
